...
If the club or upload activity does not exist, the response will contain an error (status 404):

```json
["activity:error-notFound"]
```

If the upload activity is private and the player is not a member of the club, the response will contain an error (status 403):
