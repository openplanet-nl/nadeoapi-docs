...
This endpoint is deprecated and may be removed in the future. Use the [account-specific map records (v2) endpoint](/core/records/map-records-v2-account) instead.
...
- The main difference between the [old map records endpoint](/core/deprecated/map-records) and this **v2** version is the `mapId` parameter. Using this endpoint, it is only possible to request records for one map (as opposed to requesting records for multiple players on multiple maps).
