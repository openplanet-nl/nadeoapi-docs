---
name: Get map info (multiple) by IDs

url: https://prod.trackmania.core.nadeo.online
method: GET
route: /maps/by-id/?mapIdList={mapIdList}
...
Gets information about multiple maps via their IDs.

---

**Remarks**:

- This endpoint only accepts `mapId`s - the [map info (multiple) by UID endpoint](/core/maps/info-multiple-uid) offers the same functionality for `mapUid`s.
- This endpoint has no intrinsic limit on the number of map IDs requested, but it will return a `414` error if the request URI length is 8220 characters or more (corresponding to just over 200 map IDs, depending on how you encode the URI).
...
```plain
GET https://prod.trackmania.core.nadeo.online/maps/by-id/?mapIdList=a74716be-d124-4de1-87c2-834304ccef5b,5546883f-b1ed-49e0-9397-55fc46f1d00c
```
...
If a `mapId` is invalid, that map will not be returned in the response.
